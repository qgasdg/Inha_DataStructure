...
class LinkedList: # Linked list class
    ...
    def reverse(self):  # Reverse the linked list
        ...
        while current:
            linked_node = current.link # 처음에 헤드랑 연결된 곳을 임시적으로 저장
            #거꾸로 가는 과정
            current.link = prev # 지금 값이랑 앞에꺼랑 연결해줌(연결된게 없으면 처음엔 None)
            prev = current # prev를 current로 바꿔줌
            current = linked_node # current를 다음 노드로 바꿔줌
