...
class LinkedList: # Linked list class
    ...
    def reverse_recursive(self, current, prev): # Recursive function to reverse the linked list
            if not current:
                return prev
            next = current.next
            current.next = prev
            return self.reverse_recursive(next, current) # Will return the new head
